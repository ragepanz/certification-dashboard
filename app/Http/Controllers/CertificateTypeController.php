...
class CertificateTypeController extends Controller
{
    ...
    public function show($name, Request $request)
    {
        ...
        $warningCount = Certification::where('certificate_name', $certificateName)->where(function ($q) use ($today) {
            $q->where('excel_status', 'expiring')
              ->orWhere(function ($q2) use ($today) {
                  $q2->whereNull('excel_status')
                     ->whereNotNull('expiry_date')
        ...
        $expiredCount = Certification::where('certificate_name', $certificateName)->where(function ($q) use ($today) {
            $q->where('excel_status', 'expired')
              ->orWhere(function ($q2) use ($today) {
                  $q2->whereNull('excel_status')
                     ->whereNotNull('expiry_date')
