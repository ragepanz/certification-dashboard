...
class CertificateTypeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Certification::select('certificate_name')
            ...
            ->selectRaw('SUM(CASE WHEN expiry_date > ? THEN 1 ELSE 0 END) as active_count', [$today->copy()->addDays(60)])
            ->selectRaw('SUM(CASE WHEN expiry_date >= ? AND expiry_date <= ? THEN 1 ELSE 0 END) as warning_count', [$today, $today->copy()->addDays(60)])
            ->selectRaw('SUM(CASE WHEN expiry_date < ? THEN 1 ELSE 0 END) as expired_count', [$today])
    ...
    public function show($name, Request $request)
    {
        ...
        $certifications = $query->orderBy('expiry_date', 'asc')->paginate(25)->withQueryString();
        $units = User::whereNotNull('unit')->distinct()->pluck('unit');

        $activeCount = Certification::where('certificate_name', $certificateName)->whereDate('expiry_date', '>', $today->copy()->addDays(60))->count();
        $warningCount = Certification::where('certificate_name', $certificateName)->whereDate('expiry_date', '>=', $today)->whereDate('expiry_date', '<=', $today->copy()->addDays(60))->count();
        $expiredCount = Certification::where('certificate_name', $certificateName)->whereDate('expiry_date', '<', $today)->count();
